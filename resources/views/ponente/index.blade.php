            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($ponentes as $ponente)
                    <div class="bg-white shadow-md rounded-lg overflow-hidden">
                        <img src="{{ $ponente->imagenUrl }}" alt="{{ $ponente->nombre }}" class="w-full h-48 object-cover">
                        <div class="p-4">
                            <h2 class="text-xl font-semibold">{{ $ponente->nombre }}</h2>
                            <p class="text-gray-600 mt-2">{{ implode(', ', $ponente->areas_experiencia) }}</p>
                        ...
                        <div class="flex justify-between p-4 bg-gray-100">
                            <a href="{{ route('ponentes.show', $ponente) }}" class="text-blue-600 hover:underline">Ver
                                Perfil</a>
                            <span
